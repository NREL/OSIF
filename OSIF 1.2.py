...
print('#########################################################################\n\n\n\n')
print('############################################################')
print('#####     Open Source Impedance Fitter (OSIF) v1.2     #####')
print('########################################################### #')
...
class OSIF:
    ...
    def __init__(self, master):
        master.title("Open Source Impedance Fitter (OSIF) v1.2")
        master.grid()
        ...
        Label(OutputFrame, text="Estimated SD",font =labelFont).grid(row=1, column=sdColumn,sticky=W)
        Label(OutputFrame, text="SD % of fit value",font =labelFont).grid(row=1, column=sdPerColumn,sticky=W)
        ...
        Label(OutputFrame, text="Avg. |Z| residual % of data |Z|:",font =labelFont).grid(row=8, column=(sdPerColumn-2),columnspan = 2,pady=20, sticky=E)
    ...
    def SelectDataDir(self):
        ...
        dirList = [dataFile for dataFile in dirList if ('.txt' in dataFile) | ('.xls' in dataFile) | ('.xlsx' in dataFile)]
    ...
    def LoadSElectedFile(self):
        ...
        if self.currentFileName.get().endswith('.txt'):
            ...
            while dataLineString:
                ...
                poop = re.match('^\d*.\d*\t\d*.\d*\t\d*.\d*' , dataLineString)
                if poop is not None:
                    print('----------------------------------------------------------------------------------------------------------DATA LINE BELOW')
                if poop is None:
                    print('----------------------------------------------------------------------------------------------------------NOT DATA LINE BELOW\n'+dataLineString)


                if (len(dataLineString) > 2) & (dataLineString[0] != '#') & (re.match('^\d*.\d*\t\d*.\d*\t\d*.\d*' , dataLineString) is not None) & (dataLineString != ""):
        ...
        #Load in default excel spread sheet output format from EIS software
            ...
            for real in self.activeData.rawzPrime:
                ...
                print(self.activeData.rawFrequency[i],self.activeData.rawzPrime[i],self.activeData.rawZdoublePrime[i],self.activeData.rawzMod[i])
                i += 1
    ...
    def ChopFreq(self):
        ...
        maxIndex = self.activeData.rawFrequency.tolist().index(self.activeData.frequency[self.activeData.frequency.shape[0] - 1])
    ...
    def PerformSim(self):
        ...
        if len(self.activeData.rawzPrime) == 0:
            ...
            params = [float(self.Lwire.IE.get()),float(self.Rmem.IE.get()),float(self.Rcl.IE.get()),float(self.Qdl.IE.get()),float(self.Phi.IE.get())]
    ...
    def PerformFit(self):
        ...
        if len(self.activeData.rawzPrime) == 0:
            ...
            return
                                                            ...
                                                            ### /float(self.area.IE.get())   Rmem
            self.ChopFreq()
            params = [float(self.Lwire.IE.get())/float(self.area.IE.get()),float(self.Rmem.IE.get())/float(self.area.IE.get()),float(self.Rcl.IE.get())/float(self.area.IE.get()),float(self.Qdl.IE.get()),float(self.Phi.IE.get())]

            # Perform the fitting using least_squares with the TRF method and max function calls of 10000.
            finalOutput = scipy.optimize.least_squares(self.funcCost, params, max_nfev=50000, method='trf', xtol=1e-11,ftol=1e-11, gtol=1e-11, verbose=1)
            ...
            estVars = np.matrix.diagonal(np.linalg.inv(Jacob.T.dot(Jacob)) * (np.matmul(np.transpose(finalOutput.fun), finalOutput.fun)) / (finalOutput.fun.shape[0] - self.finalParams.size))
            ...
            self.AvgRealResPer = np.sum(abs(np.array(abs(self.realFinalModel - self.activeData.zPrime)) / np.array(self.activeData.zPrime)) * 100) / \
                            self.realFinalModel.shape[0]
            self.AvgImagResPer = np.sum(abs(np.array(abs(self.imagFinalModel - self.activeData.ZdoublePrime)) / np.array(self.activeData.ZdoublePrime)) * 100) / \
            ...
            self.Lwire.OE.insert(0, '%5.8f'%self.finalParams[0])
            self.Lwire.OE.config(state='readonly')
    ...
    def CreateFigures(self,params,fitOrSim):
        ...
        nyGraph.plot(self.activeData.zPrime, self.activeData.ZdoublePrime, 'bo',ls='--', markersize = 2,linewidth=1, label='data: ' + self.activeData.dataNameNoExt)
        nyGraph.plot(self.funcreal(params), self.funcImg(params), 'ro', markersize = 2,label='\n%s\nLwire=%.5e\nRmem=%5.8f\nRcl=%5.8f\nQdl=%5.5f\nphi=%5.5f' % (graphLabel, params[0], params[1], params[2], params[3], params[4]))
        ...
        modZgraph.plot(self.activeData.frequency, self.activeData.modZExperimentalComplex, 'bo', ls='--', markersize = 2,linewidth=1 )
    ...
    def SaveData(self):
        ...
        for real in self.realFinalModel:
            dataOutFile.write(
                str(self.activeData.frequency[i]) + '\t' + str(self.realFinalModel[i]) + '\t' + str(self.imagFinalModel[i]) + '\t' + str(
        ...
        dataOutFile.write(
            ...
            '\n#\n#\tQdl/mgpt = %5.6f\n#\tL2 norm of res = %10.8f' % (
                ...
                float(self.finalParams[4]), float(self.standardDeviation[4]), self.percentSigma[4],
                float(self.finalParams[0])*float(self.area.IE.get()), float(self.standardDeviation[0])*float(self.area.IE.get()), self.percentSigma[0],
                float(self.finalParams[3]) / (float(self.area.IE.get()) * float(self.loading.IE.get())), float(self.L2NormOfRes)))
    ...
    def funcCost(self,params):
        return np.array(np.sqrt(pow((self.funcreal(params) - self.activeData.zPrime), 2) + pow((self.funcImg(params) - self.activeData.ZdoublePrime), 2)))
    ...
    def funcreal(self,param):
        return np.real(1j * param[0] * self.activeData.frequency + param[1] + pow((param[2] / (param[3] * pow((1j * 2 * np.pi * self.activeData.frequency), param[4]))), 0.5) * self.JPcoth(pow((param[2] * param[3] * pow((1j * 2 * np.pi * self.activeData.frequency), param[4])), 0.5)))

    def funcImg(self,param):
        return np.imag(1j * param[0] * self.activeData.frequency + param[1] + pow((param[2] / (param[3] * pow((1j * 2 * np.pi * self.activeData.frequency), param[4]))), 0.5) * self.JPcoth(pow((param[2] * param[3] * pow((1j * 2 * np.pi * self.activeData.frequency), param[4])), 0.5)))
