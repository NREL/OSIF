...
class OSIF:
    ...
    def __init__(self, master):
        master.title("Open Source Impedance Fitter (OSIF) v1.23")
        master.grid()
        ...
        self.Rmem.IE.insert(0, str(startRmem))
        ...
        self.Rcl.IE.insert(0, str(startRcl))
        ...
        self.Qdl.IE.insert(0, str(startQdl))
        ...
        self.Phi.IE.insert(0, str(startPhi))
        ...
        self.Lwire.IE.insert(0, str(startLwire))
        ...
        self.area.IE.insert(0, str(startArea))
        ...
        self.loading.IE.insert(0, str(startLoading))
    ...
    def LoadSElectedFile(self):
        ...
        self.activeData.HFRcorrectedPhase = []
        ...
        for i in range(len(self.activeData.rawzPrime)):
            self.activeData.rawPhase.append((180/(np.pi))*np.arctan(self.activeData.rawZdoublePrime[i]/self.activeData.rawzPrime[i]))
    ...
    def PerformFit(self):
        ...
        if len(self.activeData.rawzPrime) == 0:
            ...
            self.phaseFinalModel = (180/(np.pi))*np.arctan(self.funcImg(self.finalParams)/self.funcreal(self.finalParams))
    ...
    def SaveData(self):
        ...
        self.plotToSave.savefig((self.currentDataDir.IE.get() + self.activeData.dataNameNoExt + '_fit.png'))
        ...
        dataOutFile.write(
            '#\r#\r#\t\t\t\t   Fit values\t\t\t~std error\t\t\t   ~stdErr %% of value\n#\n#\tRmem  [ohm*cm^2] \t  = %5.8f\t\t\t%.3e\t\t\t\t%8.2f\r#\tRcl   [ohm*cm^2] \t  = %5.8f\t\t\t%.3e\t\t\t\t%8.2f' \
            '\r#\tQdl   [F/(cm^2*sec^phi)]  = %5.8f\t\t\t%.3e\t\t\t\t%8.2f\r#\tphi   [ ]  \t\t  = %5.8f\t\t\t%.3e\t\t\t\t%8.2f\r#\tLwire [H*cm^2] \t\t  = %.4e\t\t\t%.3e\t\t\t\t%8.2f' \
            '\r#\r#\tQdl/mgpt = %5.6f\r#\tL2 norm of res = %10.8f' % (
